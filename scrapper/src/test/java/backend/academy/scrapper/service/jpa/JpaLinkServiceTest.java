...
public class JpaLinkServiceTest extends IntegrationEnvironment {
    ...
    public void registerShouldCreateLinkInDatabase() {
        ...
        Mockito.when(provider.fetchInformation(Mockito.any()))
                .thenReturn(new LinkInformation(URI.create("https://example.com"), "Example", List.of()));
    ...
    public void deleteShouldRemoveLinkFromDatabase() {
        ...
        var linkId = client.sql("INSERT INTO link (url) VALUES ('https://example.com') RETURNING id")
                .query(Long.class)
                .single();
        client.sql("INSERT INTO chat_link (chat_id, link_id) VALUES (5, ?)")
                .params(linkId)
                .update();
    ...
    public void updateShouldUpdateLinkInDatabase() {
        ...
        manager.flush();
        Assertions.assertThat(client.sql("SELECT last_updated FROM link WHERE id = ?")
                        .params(linkId)
    ...
    public void getLinkSubscribersShouldReturnSubscribers() {
        ...
        client.sql("INSERT INTO chat_link (chat_id, link_id) VALUES (5, ?)")
                .params(linkId)
                .update();
        Assertions.assertThat(linkService.getLinkSubscribers(linkId)).containsExactly(5L);
    }
    ...
    public void checkNowShouldCheckLinkToDatabase() {
        ...
        manager.flush();
        Assertions.assertThat(client.sql("SELECT * FROM link WHERE id = ?")
                        .params(linkId)
                        .query(Link.class)
                        .single()
                        .lastChecked()
    ...
    public void listOldLinksShouldReturnLinks() {
        client.sql("INSERT INTO link (url, last_checked) VALUES ('https://example.com', ?)")
                .params(OffsetDateTime.now().minusDays(4))
                .update();
        client.sql("INSERT INTO link (url, last_checked) VALUES ('https://example2.com',?)")
                .params(OffsetDateTime.now().minusDays(3))
                .update();
        client.sql("INSERT INTO link (url, last_checked) VALUES ('https://example3.com', ?)")
