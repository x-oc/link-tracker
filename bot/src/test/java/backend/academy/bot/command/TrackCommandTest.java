...
public class TrackCommandTest {
    ...
    public void handleShouldTrackLink() {
        ...
        Mockito.verify(mockLinksStorage, Mockito.times(1)).addUserLink(chatId, url, null, null);
        Mockito.verify(spiedStateStorage, Mockito.times(1)).setLastLink(Mockito.anyLong(), Mockito.any());
        Mockito.verify(spiedStateStorage, Mockito.times(1)).setUserState(chatId, UserState.AWAITING_TAGS);
    }
    ...
    public void handleShouldAddTags() {
        ...
        Mockito.verify(mockLinksStorage, Mockito.times(1)).addUserLink(chatId, url, tags, null);
        Mockito.verify(mockLinksStorage, Mockito.times(1)).removeUserLink(chatId, url);
        Mockito.verify(mockUserStateStorage, Mockito.times(1)).getLastLink(chatId);
        Mockito.verify(mockUserStateStorage, Mockito.times(1)).getUserState(chatId);
    }
    ...
    public void handleShouldAddFilters() {
        ...
        Assertions.assertThat(command.handle(new CommandArguments("filter1 filter2", chatId)))
                .isEqualTo(("You started tracking the link %s!" + " You will get a notification on its' update.")
                        .formatted(url));
        Mockito.verify(mockLinksStorage, Mockito.times(1)).addUserLink(chatId, url, tags, filters);
        Mockito.verify(mockLinksStorage, Mockito.times(1)).removeUserLink(chatId, url);
        Mockito.verify(mockUserStateStorage, Mockito.times(1)).getLastLink(chatId);
        Mockito.verify(mockUserStateStorage, Mockito.times(1)).getUserState(chatId);
    }
