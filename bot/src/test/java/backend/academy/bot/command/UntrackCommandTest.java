...
public class UntrackCommandTest {
    ...
    public void handleShouldReturnErrorWhenInvalidLink() {
        ...
        Mockito.verify(spiedLinksStorage, Mockito.times(1))
            .removeUserLink(1L, "not even a link");
    }
    ...
    public void handleShouldUntrackLink() {
        ...
        Assertions.assertThat(command.handle(new CommandArguments(url, 1L)))
            .isEqualTo(("You stopped tracking the link %s! " +
                "You will no longer get notifications on its' updates.").formatted(url));
        Mockito.verify(mockedLinksStorage, Mockito.times(1))
            .removeUserLink(1L, url);
    }
}
